...
public class CameraPreview extends SurfaceView implements
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		if(!ScreenUtils.checkCameraHardware(getContext())) {
	 ...
	 * 手动设置预览方向
	 */
//	public void setCameraDisplayOrientation(boolean isPortrait){
//		if(isPortrait){
//			Log.i("rotation","旋转角度"+90);
//			camera.setDisplayOrientation(90);
//		}
//		else{
//			Log.i("rotation","旋转角度"+0);
...
//	}
	...
	private Camera.Size findPreviewSizeByScreen(Camera.Parameters parameters) {
		if (viewWidth != 0 && viewHeight != 0) {
			...
			return camera.new Size(ScreenUtils.getScreenWH(getContext()).heightPixels,
					ScreenUtils.getScreenWH(getContext()).widthPixels);
		}
	}
	...
	private  void findBestPictureSize(Camera.Parameters parameters) {
		...
		if (parameters != null)
		{
			...
			for (Camera.Size size : supprotPictureSizeList){
				...
				//宽高比：
				if (hw == PImagePicker.getDefault().getConfig().getAspectRatio()){
...
//						+"  宽高比_"+hw);
	...
	public void focusOnTouch(MotionEvent event) {
		...
		Rect focusRect = ScreenUtils.calculateTapArea(mFocusView.getWidth(),
		...
		Rect meteringRect = ScreenUtils.calculateTapArea(mFocusView.getWidth(),
	...
	public void setFocus() {
		if(!mFocusView.isFocusing()) {
			try {
				camera.autoFocus(this);
				mFocusView.setX((ScreenUtils.getWidthInPx(getContext())-mFocusView.getWidth()) / 2);
				mFocusView.setY((ScreenUtils.getHeightInPx(getContext())-mFocusView.getHeight()) / 2);
				mFocusView.beginFocus();
			} catch (Exception e) {
			}
		}
